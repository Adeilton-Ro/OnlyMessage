...
public abstract class EndpointWithResultCollection<TEndpointRequest, TApplicationRequest, TApplicationResultContent, TEndpointResponse>
    : Endpoint<TEndpointRequest>
    where TEndpointRequest : IMappeableTo<TApplicationRequest>, new()
    where TApplicationRequest : IRequestWithResult<IEnumerable<TApplicationResultContent>>
    where TEndpointResponse : IMappeableFrom<TApplicationResultContent>
{
    private readonly IMapper mapper;
    private readonly ISender sender;

    public EndpointWithResultCollection(IMapper mapper, ISender sender)
    {
        this.mapper = mapper;
        this.sender = sender;
    }
    ...
    public virtual async Task<IResult<IEnumerable<TApplicationResultContent>>> AcessApplication(TEndpointRequest req, CancellationToken cancellationToken)
    {
    ...
    public virtual Task SendResponse(IResult<IEnumerable<TApplicationResultContent>> applicationResponse, CancellationToken cancellationToken)
    {
        if (applicationResponse.IsSuccess && applicationResponse.Content is not null)
        {
            var endpointResponse = mapper.Map<IEnumerable<TApplicationResultContent>, IEnumerable<TEndpointResponse>>(applicationResponse.Content);
